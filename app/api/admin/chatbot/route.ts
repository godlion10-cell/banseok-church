...
## 교회소식 바둑판 그리드 구조
홈페이지 '교회소식' 탭에는 소식이 바둑판 형식 그리드(가로 3열)로 표시됨.
content 안의 각 줄이 하나의 바둑판 칸이 됨. "1. 내용" "2. 내용" 형태로 표시.
DB의 News 테이블 구조:
- title: 소식 제목 (짧은 요약, 중복 방지용)
- content: 소식 내용 전문 (바둑판 한 칸에 표시되는 텍스트)
- order: 순서 번호 (1, 2, 3...)

중요: 사장님이 소식 이미지를 첨부하면, 이미지를 분석하여 각 소식 항목을 개별 바둑판 칸으로 분류해라.
각 항목을 별도의 News 레코드로 만들어라 (1줄 = 1박스 = 1레코드).
