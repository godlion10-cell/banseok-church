...
export default function EumdolReportPage() {
    ...
    <div style={{ minHeight: '100vh', background: '#FDFCFB', padding: '40px 20px' }}>
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
        <header style={{ textAlign: 'center', marginBottom: '30px' }}>
          <h2 style={{ color: '#1E3A8A', fontWeight: 'bold', fontSize: '1.8rem' }}>🪨 이음돌 예배 보고</h2>
        ...
        <form onSubmit={handleSubmit} style={{ background: 'white', padding: '25px', borderRadius: '25px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div>
            ...
            <select value={eumdolName} onChange={(e) => setEumdolName(e.target.value)} required style={inputStyle}>
              ...
              <option>1이음돌 (소오비)</option>
              <option>2이음돌 (내곡)</option>
              <option>3이음돌 (중앙)</option>
              <option>4이음돌 (연초)</option>
            </select>
          </div>
            ...
            {members.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '10px', alignItems: 'center' }}>
                ...
                <input 
                  type="text" 
                  value={m.prayer} 
                  onChange={(e) => updateMember(i, 'prayer', e.target.value)} 
                  placeholder="기도제목" 
          ...
          {/* 제출 */}
          <button type="submit" disabled={sending} style={{ padding: '18px', background: sending ? '#94A3B8' : '#1E3A8A', color: 'white', border: 'none', borderRadius: '15px', fontWeight: 'bold', fontSize: '1.1rem', cursor: sending ? 'wait' : 'pointer', transition: '0.2s' }}>
            {sending ? '전송 중...' : '🪨 이음돌 보고 완료 ➤'}
          </button>
        </form>
      </div>
    </div>
